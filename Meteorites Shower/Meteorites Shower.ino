...
void displayInGameStats() {
  ...
  if (killedMeteorites % 6 == 0 && killedMeteorites != 0 && step == 1 && currentGameDifficulty == 2) {
    lcd.setCursor(4, 1);
  ...
  for (int i = 0; i < numberOfBullets; i++) {
    ...
    if (bullets[i].x >= 0 && bullets[i].x <= 7) {
      ...
      if (meteorites[bullets[i].x][bullets[i].y] >= 1) {  // the bullet is on a meteorite
        ...
        if (killedMeteorites % 6 == 0 && killedMeteorites != 0 && step == 1 && currentGameDifficulty == 2) {  // last difficulty, last step
